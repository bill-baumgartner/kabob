`{:description "This rule finds any small molecule reference record described in Reactome with a PubChem ID, as opposed to a ChEBI one.",
 :name "step_cab-add_reactome_small_molecule_references_with_pubchem_ids_to_ice",
 :reify ([?/smmol_ref_record {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 "Reactome small molecule reference record" ?/smmol_ref), :prefix "R_"}]
         [?/this_xref_record {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 ?/xref_record ?/smmol_ref_record), :prefix "R_"}]
         ),
  :head ((?/smmol_ref_record rdf/type ccp/IAO_EXT_0001552) ;; small molecule reference
         ...
         (?/this_xref_record rdfs/subClassOf ?/xref_record) ;; xref field
...
SELECT DISTINCT ?smmol_ref ?xref_record ?xref_id_field ?pubchem_ice WHERE {
