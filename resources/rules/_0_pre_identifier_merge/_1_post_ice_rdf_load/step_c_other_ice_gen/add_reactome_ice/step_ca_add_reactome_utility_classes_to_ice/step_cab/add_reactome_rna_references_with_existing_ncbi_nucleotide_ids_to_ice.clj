`{:description "This rule finds any rna reference record described in Reactome with an existing NCBI nucleotide ID via an HGNC id. It may miss names that don't match the HGNC id.",
 :name "step_cab-add_reactome_rna_references_with_existing_ncbi_nucleotide_ids_to_ice",
 :reify ([?/rna_ref_record {:ns "kice", :ln (:sha-1 "Reactome rna reference record" ?/rna_ref), :prefix "R_"}]),
...
SELECT ?rna_ref ?xref ?xref_record ?xref_id_field ?refseq_id WHERE {
