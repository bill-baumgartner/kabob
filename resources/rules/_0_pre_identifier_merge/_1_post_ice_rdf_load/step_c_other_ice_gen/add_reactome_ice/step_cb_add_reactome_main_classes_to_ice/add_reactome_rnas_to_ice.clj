`{:description "This rule finds any RNA record described in Reactome, as well as its cellular location and its Reactome id field.",
 :name "step_cb-add_reactome_rnas_to_ice",
 :reify ([?/rna_record {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 "Reactome rna record" ?/rna), :prefix "R_"}]
         [?/this_xref_record {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 ?/xref_record ?/rna_record), :prefix "R_"}]
         [?/reactome_ice]
         ),
...
SELECT DISTINCT ?record_set ?rna ?xref_record ?xref_id_field ?reactome_ice WHERE {
