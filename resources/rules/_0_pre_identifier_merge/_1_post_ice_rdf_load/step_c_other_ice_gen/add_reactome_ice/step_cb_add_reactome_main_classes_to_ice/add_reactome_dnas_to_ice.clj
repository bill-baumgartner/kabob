`{:description "This rule finds any DNA record described in Reactome, as well as its Reactome id field.",
 :name "step_cb-add_reactome_dnas_to_ice",
 :reify ([?/dna_record {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 "Reactome dna record" ?/dna), :prefix "R_"}]
         [?/this_xref_record {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 ?/xref_record ?/dna_record), :prefix "R_"}]
         [?/reactome_ice]
         ),
...
SELECT DISTINCT ?record_set ?dna ?xref_record ?xref_id_field ?reactome_ice WHERE {
