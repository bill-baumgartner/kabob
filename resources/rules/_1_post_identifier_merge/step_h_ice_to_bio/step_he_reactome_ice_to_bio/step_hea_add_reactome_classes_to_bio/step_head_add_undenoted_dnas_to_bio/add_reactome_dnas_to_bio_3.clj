`{:description "This rule finds any unmodified dna record described in Reactome.  The rule places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the ENSEMBL or NCBI Nucleotide ICE id.  It also makes the denotational link to a new BIO entity if one doesn't exist already.",
 ...
 :body "#add_reactome_dnas_to_bio_3
...
SELECT DISTINCT ?reactome_ice ?external_ice ?accession_label ?transports_bio ?targets_bio ?localization_bio ?go_cc_bio ?dna_record 
WHERE {
...
# make sure that this is not a DNA variant
OPTIONAL {
