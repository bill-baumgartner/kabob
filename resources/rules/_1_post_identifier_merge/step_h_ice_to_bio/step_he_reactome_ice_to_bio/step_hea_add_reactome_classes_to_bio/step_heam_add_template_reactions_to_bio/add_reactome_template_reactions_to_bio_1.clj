`{:description "This rule finds any template reaction record described in Reactome lacking a GO BP annotation and puts it on the bio side.",
 :name "add_reactome_template_reactions_to_bio_1",
 :reify ([?/this_tr {:ln (:sha-1 ?/tr_record), :ns "http://ccp.ucdenver.edu/kabob/bio/", :prefix "BP_"}]
         ),
 :head
 ((?/react_ice obo/IAO_0000219 ?/this_tr) 
  (?/this_tr rdfs/subClassOf ?/go_bp)  ;; general GO-BP
  ),
 :body "#add_reactome_template_reactions_to_bio_1.clj
...
SELECT DISTINCT ?tr_record ?go_bp ?react_ice 
WHERE {   
?tr_record rdf:type ccp:IAO_EXT_0001580 . # template reaction record
?tr_record obo:BFO_0000051 ?this_xref_record .
...
OPTIONAL {
?tr_record obo:BFO_0000051 ?this_go_bp_xref_record .
