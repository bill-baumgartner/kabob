`{:description "This rule finds any unmodified rna record described in Reactome.  The rule places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the ENSEMBL or NCBI Nucleotide ICE id.  It also makes the denotational link to a new BIO entity if one doesn't exist already.",
 :name "add_reactome_rnas_to_bio_3",
 :reify ([?/original_foo {:ln (:sha-1 "bio-side reactome rna" ?/rna_record), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
         [?/external_bio {:ln (:sha-1 ?/reactome_ice ?/accession_label), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
 ...
 :body "#add_reactome_rnas_to_bio_3
...
SELECT DISTINCT ?reactome_ice ?external_ice ?accession_label ?transports_bio ?targets_bio ?localization_bio ?go_cc_bio ?rna_record 
WHERE {
...
# make sure that this is not an RNA variant
OPTIONAL {
?rna_record obo:BFO_0000051 ?this_feature_record .
?this_feature_record rdf:type ccp:IAO_EXT_0001527 . 
}
filter (!bound (?this_feature_record)) .
