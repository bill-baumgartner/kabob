`{:description "This rule finds any biochemical reaction record described in Reactome with a double location (i. e., a transport) and puts it on the bio side.",
 :name "add_reactome_biochemical_reaction_locations_to_bio_2",
 :reify ([?/this_input_go_cc_bio {:ln (:sha-1 ?/input_go_cc_bio ?/bcr_bio), :ns "http://ccp.ucdenver.edu/kabob/bio/", :prefix "B_"}]
         [?/occurs_in_restriction_1 {:ln (:restriction), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "RS_"}]
         [?/this_output_go_cc_bio {:ln (:sha-1 ?/output_go_cc_bio ?/bcr_bio), :ns "http://ccp.ucdenver.edu/kabob/bio/", :prefix "B_"}]
         [?/occurs_in_restriction_2 {:ln (:restriction), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "RS_"}]
         ),
 :head
 ((?/bcr_bio rdfs/subClassOf ?/occurs_in_restriction_1)
  (?/this_input_go_cc_bio rdfs/subClassOf ?/input_go_cc_bio)
  (?/occurs_in_restriction_1 rdf/type owl/Restriction)
  (?/occurs_in_restriction_1 owl/onProperty ?/occurs_in_bio)
  (?/occurs_in_restriction_1 owl/someValuesFrom ?/this_input_go_cc_bio)
  (?/bcr_bio rdfs/subClassOf ?/occurs_in_restriction_2)
 ...
 :body "#add_reactome_biochemical_reaction_locations_to_bio_2.clj
...
SELECT ?bcr_bio ?input_go_cc_bio ?output_go_cc_bio ?occurs_in_bio
WHERE {   
