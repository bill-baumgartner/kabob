`{:description "This rule finds any physical entity record described in Reactome and adds its label (display name + location) to the bio side.",
 :name "add_reactome_physical_entity_labels_to_bio",
 :head
 ((?/physical_entity_bio rdfs/label ?/physical_entity_label) 
  ),
 :body "#add_reactome_physical_entity_labels_to_bio.clj
...
SELECT DISTINCT ?physical_entity_bio ?physical_entity_label 
WHERE {   
?physical_entity_record rdf:type ccp:IAO_EXT_0001515 .

?physical_entity_record obo:BFO_0000051 ?this_reactome_xref_record .
...
?reactome_ice obo:IAO_0000219 ?physical_entity_bio .
filter (contains (str (?physical_entity_bio), \"http://ccp.ucdenver.edu/kabob/bio/\")) .
?physical_entity_record obo:BFO_0000051 ?display_name_field .
...
bind (concat (str (?display_name_label), \" (in \", str(?gocc_label), \")\") AS ?physical_entity_label). 
}",
