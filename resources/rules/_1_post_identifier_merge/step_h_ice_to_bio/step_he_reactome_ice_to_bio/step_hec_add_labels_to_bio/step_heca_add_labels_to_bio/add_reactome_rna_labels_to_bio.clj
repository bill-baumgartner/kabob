`{:description "This rule finds any rna record described in Reactome and adds its label (display name + location) to the bio side.",
 :name "add_reactome_rna_labels_to_bio",
 :head
 ((?/rna_bio rdfs/label ?/rna_label) 
  ),
 :body "#add_reactome_rna_labels_to_bio.clj
...
SELECT DISTINCT ?rna_bio ?rna_label 
WHERE {   
?rna_record rdf:type ccp:IAO_EXT_0001558 .

?rna_record obo:BFO_0000051 ?this_reactome_xref_record .
...
?reactome_ice obo:IAO_0000219 ?rna_bio .
filter (contains (str (?rna_bio), \"http://ccp.ucdenver.edu/kabob/bio/\")) .
?rna_record obo:BFO_0000051 ?display_name_field .
...
bind (concat (str (?display_name_label), \" (in \", str(?gocc_label), \")\") AS ?rna_label). 
}",
