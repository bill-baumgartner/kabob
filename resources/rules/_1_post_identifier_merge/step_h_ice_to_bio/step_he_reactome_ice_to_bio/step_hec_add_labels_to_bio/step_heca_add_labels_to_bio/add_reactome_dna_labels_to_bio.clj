`{:description "This rule finds any dna record described in Reactome and adds its label (display name + location) to the bio side.",
 :name "add_reactome_dna_labels_to_bio",
 :head
 ((?/dna_bio rdfs/label ?/dna_label) 
  ),
 :body "#add_reactome_dna_labels_to_bio.clj
...
SELECT DISTINCT ?dna_bio ?dna_label 
WHERE {   
?dna_record rdf:type ccp:IAO_EXT_0001556 .

?dna_record obo:BFO_0000051 ?this_reactome_xref_record .
...
?reactome_ice obo:IAO_0000219 ?dna_bio .
filter (contains (str (?dna_bio), \"http://ccp.ucdenver.edu/kabob/bio/\")) .
?dna_record obo:BFO_0000051 ?display_name_field .
...
bind (concat (str (?display_name_label), \" (in \", str(?gocc_label), \")\") AS ?dna_label). 
}",
